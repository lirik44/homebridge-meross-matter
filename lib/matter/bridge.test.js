...
describe('what is published at all', () => {
  ...
  it('publishes under a fresh identity when the suffix is changed', async () => {
    // What a controller keeps on its own side about an accessory - Apple Home's choice to split
    // a composed device into separate tiles - goes away only when the accessory itself is new.
    const platform = fakePlatform([powerStrip()], { matterIdSuffix: 'b' })
    const bridge = new MatterBridge(platform)
    const before = { UUID: 'uuid:meross-matter-sn-1-0', displayName: 'Power Strip' }

    bridge.rememberCached(before)
    await bridge.publish('plugin', 'platform')
    const [[,, [descriptor]]] = platform.api.matter.registerPlatformAccessories.mock.calls

    expect(descriptor.UUID).toBe('uuid:meross-matter-sn-1-0-b')
    // And the one the controllers had is taken away in the same breath.
    expect(platform.api.matter.unregisterPlatformAccessories).toHaveBeenCalledWith('plugin', 'platform', [before])
  })
...
describe('the energy experiment', () => {
  ...
  it('publishes both the running total and the window it covers', async () => {
    ...
    expect(energy.cumulativeEnergyImported.startTimestamp).toBe(Math.floor(Date.parse('2026-09-21T00:00:00Z') / 1000))
  })

  it('never reports a window of no width, which the endpoint refuses', async () => {
    // The first reading is taken the moment the socket is published, so both ends of its window
    // are the same instant - and an endpoint that refuses one attribute refuses the accessory.
    const { platform } = metered()
    await new MatterBridge(platform).publish('plugin', 'platform')
    const energy = readings(platform)[0].clusters.electricalEnergyMeasurement

    expect(energy.cumulativeEnergyImported.endTimestamp).toBeGreaterThan(energy.cumulativeEnergyImported.startTimestamp)
    expect(energy.periodicEnergyImported.endTimestamp).toBeGreaterThan(energy.periodicEnergyImported.startTimestamp)
  })
  ...
  it('asks the platform to keep the totals each time it reports', async () => {
    vi.useFakeTimers()
    try {
      const platform = fakePlatform([powerStrip()])
      platform.energy = new EnergySimulator()
      platform.saveEnergyTotals = vi.fn()
      await new MatterBridge(platform).publish('plugin', 'platform')

      await vi.advanceTimersByTimeAsync(60_000)

      expect(platform.saveEnergyTotals).toHaveBeenCalled()
    } finally {
      vi.useRealTimers()
    }
  })
