...
function fakeService(uuid, subtype, displayName, value, apply, readings = {}) {
  const characteristic = new FakeCharacteristic(value, apply)
  // Eve's energy characteristics, when the socket carries them.
  const extra = new Map(Object.entries(readings).map(([name, reading]) => [name, new FakeCharacteristic(reading)]))
  ...
  return {
    ...
    characteristic,
    extra,
    testCharacteristic: char => char === 'On' || extra.has(char),
    getCharacteristic: char => (char === 'On' ? characteristic : extra.get(char)),
  }
}
...
function fakePlatform(accessories, config = {}) {
  ...
  return {
    ...
    log,
    config,
    eveChar: { CurrentConsumption: 'W', TotalConsumption: 'kWh', Voltage: 'V', ElectricCurrent: 'A' },
...
describe('the energy experiment', () => {
  // One socket, drawing the made-up 1.13 kW, with a total behind it.
  function metered(watts = 1130, kWh = 2.5) {
    return fakeAccessory('hb-1', 'Power Strip', [
      fakeService(OUTLET, 'outlet-1', 'Xbox', true, undefined, {
        W: watts,
        kWh,
        V: 230,
        A: 4.91,
      }),
      fakeService(OUTLET, 'outlet-2', 'AppleTV', false, undefined, {
        W: 0,
        kWh: 0,
        V: 0,
        A: 0,
  ...
  it('publishes the readings as matter clusters, in matter units', async () => {
    const platform = fakePlatform([metered()])
    await new MatterBridge(platform).publish('plugin', 'platform')
    const [[,, [descriptor]]] = platform.api.matter.registerPlatformAccessories.mock.calls

    // Milliwatts, millivolts, milliamps and milliwatt-hours, as the spec has them.
    expect(descriptor.parts[0].clusters.electricalPowerMeasurement).toEqual({
      activePower: 1_130_000,
      voltage: 230_000,
      activeCurrent: 4910,
    })
    expect(descriptor.parts[0].clusters.electricalEnergyMeasurement).toEqual({
      cumulativeEnergyImported: { energy: 2_500_000 },
    })
  })

  it('publishes zeroes for a socket that is off rather than leaving it out', () => {
    const platform = fakePlatform([metered()])
    const bridge = new MatterBridge(platform)
    const descriptor = bridge.descriptorFor(platform.devicesInHB.get('hb-1'))

    expect(descriptor.parts[1].clusters.electricalPowerMeasurement.activePower).toBe(0)
  })

  it('reports a new reading to the controllers', async () => {
    const platform = fakePlatform([metered()])
    await new MatterBridge(platform).publish('plugin', 'platform')
    const accessory = platform.devicesInHB.get('hb-1')

    accessory.services[0].extra.get('W').change(1500)

    expect(platform.api.matter.updateAccessoryState).toHaveBeenCalledWith(
      'uuid:meross-matter-sn-1-0',
      'electricalPowerMeasurement',
      { activePower: 1_500_000, voltage: 230_000, activeCurrent: 4910 },
      'outlet-1',
    )
  })

  it('reports a total that has climbed', async () => {
    const platform = fakePlatform([metered()])
    await new MatterBridge(platform).publish('plugin', 'platform')
    const accessory = platform.devicesInHB.get('hb-1')

    accessory.services[0].extra.get('kWh').change(3.25)

    expect(platform.api.matter.updateAccessoryState).toHaveBeenCalledWith(
      'uuid:meross-matter-sn-1-0',
      'electricalEnergyMeasurement',
      { cumulativeEnergyImported: { energy: 3_250_000 } },
      'outlet-1',
    )
  })

  it('does not send a reading the controllers already have', async () => {
    const platform = fakePlatform([metered()])
    await new MatterBridge(platform).publish('plugin', 'platform')
    const accessory = platform.devicesInHB.get('hb-1')

    accessory.services[0].extra.get('W').change(1500)
    const first = platform.api.matter.updateAccessoryState.mock.calls.length
    accessory.services[0].extra.get('W').change(1500)

    expect(platform.api.matter.updateAccessoryState.mock.calls).toHaveLength(first)
  })

  it('leaves the clusters out for a socket that measures nothing', async () => {
    const platform = fakePlatform([powerStrip()])
    await new MatterBridge(platform).publish('plugin', 'platform')
    const [[,, [descriptor]]] = platform.api.matter.registerPlatformAccessories.mock.calls

    expect(descriptor.parts[0].clusters.electricalPowerMeasurement).toBeUndefined()
    expect(descriptor.parts[0].clusters.electricalEnergyMeasurement).toBeUndefined()
  })
})
