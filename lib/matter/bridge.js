...
export default class MatterBridge {
    ...
    return {
      ...
      electricalEnergyMeasurement: {
        ...
        cumulativeEnergyImported: {
          ...
          startTimestamp: seconds(reading.startedAt),
          endTimestamp: seconds(reading.periodEnd),
        },
        periodicEnergyImported: {
          energy: Math.round(reading.periodKWh * 1_000_000),
          startTimestamp: seconds(reading.periodStart),
          endTimestamp: seconds(reading.periodEnd),
        },
      },
    }
  ...
  startEnergyReporting() {
    ...
    this.energyTimer = setInterval(() => {
      this.published.forEach((parts, uuid) => {
        parts.forEach(({ partId, service }) => this.reportEnergy(uuid, partId, service))
      })
    }, ENERGY_INTERVAL_MS)
  }
