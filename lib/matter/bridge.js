...
export default class MatterBridge {
  ...
  descriptorFor(accessory) {
    ...
    const uuid = this.api.matter.uuid.generate(
      `meross-matter-${plan.serialNumber}-${accessory.context?.channel ?? 0}`,
    )
    ...
    if (!plan.composed) {
      ...
      return {
        ...
        clusters: { onOff: { onOff: part.on }, ...this.energyClusters(part.ref) },
  ...
  /**
   * The energy experiment. The readings live on the HomeKit service as Eve's characteristics, so
   * Matter is fed from the same place HomeKit is and the two cannot drift apart.
   *
   * @param {object} service The HomeKit service behind a socket.
   * @returns {object|null} What it reports, or null when it reports nothing.
   */
  energyOf(service) {
    const eve = this.platform.eveChar
    if (!eve || !service.testCharacteristic(eve.CurrentConsumption)) {
      return null
    }

    const read = (char) => {
      if (!service.testCharacteristic(char)) {
        return 0
      }
      const value = service.getCharacteristic(char).value
      return typeof value === 'number' ? value : 0
    }

    return {
      watts: read(eve.CurrentConsumption),
      kWh: read(eve.TotalConsumption),
      volts: read(eve.Voltage),
      amps: read(eve.ElectricCurrent),
    }
  }
   ...
   *   millivolts, current in milliamps, energy in milliwatt-hours, as the spec has them.
   *   Homebridge fills in the mandatory accuracy and power-mode metadata itself.
   */
  energyClusters(service) {
    const reading = this.energyOf(service)
    if (!reading) {
      return {}
    }
    ...
    return {
      ...
      electricalEnergyMeasurement: {
        cumulativeEnergyImported: { energy: Math.round(reading.kWh * 1_000_000) },
      },
    }
  }
    ...
    this.published.forEach((parts, uuid) => {
      parts.forEach(({ partId, service }) => {
        ...
        if (!this.followed.has(char)) {
          ...
          char.on('change', ({ newValue }) => this.report(uuid, partId, newValue === true))
        }

        // The energy readings move on their own - a socket left on keeps adding to its total - so
        // they are followed separately from the switch.
        const energyChars = eve
          ? [eve.CurrentConsumption, eve.TotalConsumption, eve.Voltage, eve.ElectricCurrent]
          : []

        energyChars
          .filter(energyChar => energyChar && service.testCharacteristic(energyChar))
          .forEach((energyChar) => {
            const characteristic = service.getCharacteristic(energyChar)
            if (this.followed.has(characteristic)) {
              return
            }

            this.followed.add(characteristic)
            characteristic.on('change', () => this.reportEnergy(uuid, partId, service))
          })
      })
    })
  }
  ...
  reportEnergy(uuid, partId, service) {
    const clusters = this.energyClusters(service)
