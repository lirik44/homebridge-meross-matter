...
export default class MatterBridge {
  ...
  report(uuid, partId, on) {
    const key = `${uuid}:${partId ?? ''}`
    if (this.reported.get(key) === on) {
      return
    }
    this.reported.set(key, on)

    const cluster = this.api.matter.clusterNames?.OnOff ?? 'onOff'
    Promise.resolve(this.api.matter.updateAccessoryState(uuid, cluster, { onOff: on }, partId))
  ...
  remember(uuid, parts) {
    ...
    parts.forEach(({ partId, on }) => this.reported.set(`${uuid}:${partId ?? ''}`, on))
  }
