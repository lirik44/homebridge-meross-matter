...
export default class {
  ...
  async login() {
    try {
      ...
      if (res.data.apiStatus === 1030) {
        this.domain = res.data.data.domain.replace(httpProtocolRegex, '')
  ...
  async getSubDevices(device) {
    try {
      ...
      if (err.code && platformConsts.httpRetryCodes.includes(err.code)) {
        // Retry if another attempt could be successful
        this.log.warn('[HTTP] %s [getSubDevices() - %s].', platformLang.httpRetry, err.code)
        await sleep(30000)
        return this.getSubDevices(device)
      }
      throw err
    }
  }
