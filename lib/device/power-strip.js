...
export default class {
  constructor(platform, accessory) {
    ...
    for (let i = 1; i < accessory.context.channelCount; i += 1) {
      ...
      // The experiment: Eve's energy characteristics on every socket, fed with invented numbers.
      // The strip measures nothing, so this is only here to see what the Home app draws for them.
      if (!outletService.testCharacteristic(this.eveChar.CurrentConsumption)) {
        outletService.addCharacteristic(this.eveChar.CurrentConsumption)
      }
      if (!outletService.testCharacteristic(this.eveChar.TotalConsumption)) {
        outletService.addCharacteristic(this.eveChar.TotalConsumption)
      }
      if (!outletService.testCharacteristic(this.eveChar.Voltage)) {
        outletService.addCharacteristic(this.eveChar.Voltage)
      }
      if (!outletService.testCharacteristic(this.eveChar.ElectricCurrent)) {
        outletService.addCharacteristic(this.eveChar.ElectricCurrent)
      }
      if (!outletService.testCharacteristic(this.eveChar.ResetTotal)) {
        outletService.addCharacteristic(this.eveChar.ResetTotal)
        outletService.getCharacteristic(this.eveChar.ResetTotal).onSet(() => {
          this.platform.energy.reset(`outlet-${i}`)
          outletService.updateCharacteristic(this.eveChar.TotalConsumption, 0)
        })
      }
    }

    // Energy readings climb with time rather than with anything the device says, so they are
    // pushed on a measured cadence of their own as well as whenever a socket is switched.
    this.accessory.powerInterval = setInterval(() => this.reportEnergy(), 60000)
  ...
  /**
   * Tells HomeKit what each socket is pretending to draw, in Eve's characteristics. The Matter
   * side reads these same characteristics, so both ecosystems get one story.
   *
   * @returns {void}
   */
  reportEnergy() {
    for (let i = 1; i < this.accessory.context.channelCount; i += 1) {
      ...
      const on = outletService.getCharacteristic(this.hapChar.On).value === true
      const { watts, kWh, volts, amps } = this.platform.energy.sample(`outlet-${i}`, on)

      outletService.updateCharacteristic(this.hapChar.OutletInUse, on)
      outletService.updateCharacteristic(this.eveChar.CurrentConsumption, watts)
      outletService.updateCharacteristic(this.eveChar.TotalConsumption, Math.round(kWh * 1000) / 1000)
      outletService.updateCharacteristic(this.eveChar.Voltage, on ? volts : 0)
      outletService.updateCharacteristic(this.eveChar.ElectricCurrent, amps)
    }
  }
}
