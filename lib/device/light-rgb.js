    ...
    if (data.light) {
      ...
      if (hasProperty(data.light, 'rgb') && [1, 5].includes(data.light.capacity)) {
        ...
        if (this.cacheHue !== newHue || this.cacheSat !== newSat) {
          ...
          // Somebody chose this colour outside HomeKit - in the Meross app, on
          // a remote, or at the wall. Homebridge catches changes made in the
          // Home app itself, but nothing else sees these, so adaptive lighting
          // would carry on and move the light off that colour within minutes.
          //
          // This used to sit outside the check above and fire on every report,
          // so a routine refresh that simply repeated the current colour back
          // switched adaptive lighting off seconds after it was switched on.
      ...
      if (
        ...
        if (this.cacheMired !== hkTemp) {
          ...
          // A light repeating back the temperature adaptive lighting just set
          // can land a few mireds off through the conversion, and treating that
          // as the owner's doing would switch adaptive lighting off moments
          // after it was switched on. Further apart than that and somebody set
          // this temperature themselves, outside HomeKit - which nothing else
          // would notice, so adaptive lighting has to be stopped here.
          if (dif > 10 && this.alController?.isAdaptiveLightingActive?.()) {
            this.alController.disableAdaptiveLighting()
            this.accessory.log(platformLang.alDisabled)
          }
        }
