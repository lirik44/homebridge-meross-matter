...
export default class {
  ...
  /**
   * Cancel the quick polling started when the door was told to close. Kept in one
   * place so every path that ends a close - a new operation, the door settling,
   * or Homebridge shutting down - stops it rather than leaving it running.
   */
  stopExtremePolling() {
      ...
      await this.queue.add(async () => {
        ...
        */
        if (action === 'open') {
          ...
          // A new operation supersedes any close still in progress, so its quick
          // polling has to stop here. Without this it was orphaned: the close
          // timeout returned early on the changed key and never reached its own
          // clearInterval, so the device was polled every 3 seconds for ever, and
          // each open-during-close cycle leaked another one.
          this.stopExtremePolling()
