...
/**
 * Find the nested object in a hub sub-device entry that carries its sensor
 * readings.
 *
 * Meross names that object after the model — `ms100` on an MS100, `ms130` on an
 * MS130 — so keying on one name means every new sensor silently reports
 * nothing. Match on the fields instead, which covers the models that exist and
 * the ones that do not yet.
 *
 * @param {object} subdevice a `digest.hub.subdevice` entry
 * @returns {object|undefined} the readings object, if there is one
 */
export function readingsFrom(subdevice) {
  return Object.values(subdevice || {}).find(value => value
    && typeof value === 'object'
    && !Array.isArray(value)
    && (hasProperty(value, 'latestTemperature') || hasProperty(value, 'latestHumidity')))
}
      ...
      await this.queue.add(async () => {
        ...
        if (data.all) {
          if (
            ...
            // Rebuild these each poll. They used to be appended to and never
            // cleared, so with one valve on the hub and the default 300s refresh
            // the follow-up request carried 288 copies of the same id after a day,
            // growing until the hub stopped answering and state stopped updating.
            this.mtsList = []
            this.waterList = []
            ...
            data.all.digest.hub.subdevice.forEach((subdevice) => {
              ...
              } else if (readingsFrom(subdevice)) {
                // Newer sensors wrap their readings in an object named after the
                // model (an MS130 reports under `ms130`, not `ms100`), so match
                // on the fields rather than adding a branch per model — that is
                // what left the MS130 with an accessory but no data.
                const readings = readingsFrom(subdevice)
                const update = {}
                if (hasProperty(readings, 'latestTemperature')) {
                  update.temperature = readings.latestTemperature
                }
                if (hasProperty(readings, 'latestHumidity')) {
                  update.humidity = readings.latestHumidity
                }
                if (hasProperty(readings, 'voltage')) {
                  update.voltage = readings.voltage
                }
                subAcc.control.applyUpdate(update)
