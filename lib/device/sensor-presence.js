...
export default class {
  ...
  receiveUpdate(params) {
    try {
      // Log the received data
      // At debug, like every other handler. This sensor pushes a message every
      // few seconds and the whole message is logged, including the header's
      // `sign` - a value derived from the owner's device key - so at warn level
      // it filled the log with traffic nobody asked to see.
  ...
  applyUpdate(data) {
    // Compared against undefined rather than checked for truth: zero lux is a
    // real reading from a dark room, and skipping it left the sensor showing
    // whatever the light level was when the lights went out.
    if (data.light?.[0]?.value !== undefined) {
