...
describe('energySimulator', () => {
  ...
  it('carries the totals over a restart, without counting the time it was down', () => {
    const { sim, clock } = simulator()

    sim.sample('outlet-1', true)
    clock.minutes = 60
    sim.sample('outlet-1', true)
    const kept = sim.snapshot()

    // A new process, an hour later.
    const restarted = new EnergySimulator({ now: () => atMinute(120) })
    restarted.restore(kept)

    const reading = restarted.sample('outlet-1', true)
    expect(reading.kWh).toBeCloseTo(1.13, 5)
  })

  it('keeps when each socket started counting, so the reading covers the right stretch', () => {
    const { sim, clock } = simulator()
    ...
    expect(restarted.sample('outlet-1', true).startedAt).toBe(atMinute(0))
  })

  it('ignores anything unusable in what was kept', () => {
    const sim = new EnergySimulator()
    sim.restore({ 'outlet-1': { kWh: 'lots' }, 'outlet-2': null, 'outlet-3': { kWh: 2 } })

    expect(sim.sample('outlet-1', true).kWh).toBe(0)
    expect(sim.sample('outlet-3', true).kWh).toBe(2)
  })

  it('copes with nothing kept at all', () => {
    const sim = new EnergySimulator()
    sim.restore(undefined)
