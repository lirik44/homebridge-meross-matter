...
export default class EnergySimulator {
   ...
   * @returns {{watts: number, kWh: number, volts: number, amps: number}} What to report.
   */
  sample(key, on) {
    const at = this.now()
    const before = this.sockets.get(key) ?? { on, kWh: 0, at }

    // Only the time a socket spent switched on counts towards its total.
    const hours = Math.max(0, at - before.at) / 3_600_000
    const kWh = before.kWh + (before.on ? (this.watts / 1000) * hours : 0)

    this.sockets.set(key, { on, kWh, at })
   ...
   */
  reset(key) {
    this.sockets.set(key, { on: this.sockets.get(key)?.on ?? false, kWh: 0, at: this.now() })
  }
}
