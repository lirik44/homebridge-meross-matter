...
// Where the energy experiment's running totals are kept between restarts.
const ENERGY_STORAGE_KEY = 'Meross_Energy_Totals'
...
export default class {
  constructor(log, config, api) {
    ...
    try {
      ...
      this.matter = new MatterBridge(this)

      // The energy experiment: invented readings, shared by both ecosystems so they agree
      this.energy = new EnergySimulator()
  ...
  applyUserConfig(config) {
    ...
    const logDecrease = (k, max) => {
      this.log.warn('%s [%s] %s %s.', platformLang.cfgItem, k, platformLang.cfgHigh, max)
    }
    const logQuotes = (k) => {
        ...
        case 'thermostatDevices':
          if (Array.isArray(val) && val.length > 0) {
            val.forEach((x) => {
              ...
              // An entry with just a serial number carries nothing; two keys
              // (serial number plus name) is kept, as that is what the UI
              // device picker creates for users to build on
              if (entries.length < 2) {
                logRemove(`${key}.${id}`)
                return
              }
  ...
  async pluginSetup() {
    ...
    try {
      ...
      // Take back up the energy totals from before the restart, so a controller does not read the
      // counter starting over as the counter having been reset
      try {
        this.energy.restore(await this.storageData?.getItem(ENERGY_STORAGE_KEY))
      } catch (err) {
        this.log.debugWarn('Could not read the stored energy totals: %s.', parseError(err))
      }

      // If the user has configured cloud username and password then get a device list
      this.accountDetails = {}
      let cloudDevices = []
      let cloudListFetched = false
      try {
      ...
      this.devicesInHB.forEach((accessory) => {
        switch (accessory.context.connection) {
          case 'cloud':
          case 'hybrid':
            // Only a list that actually arrived says anything about what is on the
            // account. `cloudDevices` is also empty when the login or the device
            // request failed, and an owner with a user key set reaches this code
            // in that case - so one bad morning at Meross used to wipe their whole
            // setup out of HomeKit, rooms, scenes and automations included.
            if (cloudListFetched && !cloudDevices.some(el => el.uuid === accessory.context.serialNumber)) {
      ...
      // Publish whatever HomeKit ended up with over Matter as well
      await this.matter.publish(plugin.name, plugin.alias)
  ...
  pluginShutdown() {
    ...
    try {
      // Stop every accessory's timers. Only the mqtt disconnect needs a cloud
      // client: this whole block used to sit behind that check, so a local-only
      // setup - no username or password, just a user key - kept polling every
      // device while Homebridge was tearing down, and kept the process alive
      this.devicesInHB.forEach((accessory) => {
        if (this.cloudClient && accessory.mqtt) {
        ...
        accessory.control?.stopExtremePolling?.()
      })
      this.matter?.stop()
      this.saveEnergyTotals()
    } catch (err) {
  ...
  async initialiseDevice(device) {
    try {
      ...
      if (platformConsts.models.switchSingle.includes(device.model)) {
        ...
        accessory.context = { ...accessory.context, ...context }
        this.applyAccessoryLogging(accessory)
        accessory.control = ['6.0.0', '7.0.0', '8.0.0', '9.0.0'].includes(device.hdwareVersion)
        ...
        /** */
      } else if (platformConsts.models.thermostatMts215.includes(device.model)) {
        /**
         ********
         THERMOSTATS (MTS215/MTS215B)
         *********
         */
        accessory = this.devicesInHB.get(hbUUID) || this.addAccessory(device)
        accessory.context = { ...accessory.context, ...context }
        this.applyAccessoryLogging(accessory)
        accessory.control = new deviceTypes.deviceThermostatMts215(this, accessory)
        /** */
        ...
        subdevices.forEach((subdevice) => {
          try {
            ...
            switch (subdeviceObj.model) {
              // MA151 pairs with the MSH450 hub. It is grouped with the GS559A
              // because both are smoke alarms, but note what that currently
              // buys: this handler exposes a battery only. Which field an MA151
              // uses to report an alarm is not known yet (#803), so it appears
              // in HomeKit and reports battery, and the smoke state still has
              // to be mapped once a real payload has been seen.
              case 'GS559A':
              case 'MA151':
                subAcc.control = new deviceTypes.deviceHubSmoke(this, subAcc)
                break
              // MS130/MS130H are temperature + humidity sensors like the MS100,
              // so they share its handler. They were already declared supported
              // but had no case here, which meant the accessory was added and
              // then silently dropped by `default` below - leaving an empty
              // accessory and no explanation.
              case 'MS100':
              case 'MS100F':
              case 'MS130':
              case 'MS130H':
              ...
              case 'MTS150P':
                ...
                break
              default:
                // Reached when a model is listed as supported but has no case
                // above. The accessory has already been added at this point, so
                // returning quietly leaves an empty one behind with nothing to
                // explain it — which is exactly how the MS130 gap went unnoticed.
                this.log.warn(
                  '[%s] %s:\n%s',
                  subdeviceObj.devName,
                  platformLang.notSupp,
  ...
  addExternalAccessory(device, category) {
    try {
      ...
      // platform so every cache save then fails to serialize it (#792)
  ...
  async saveEnergyTotals() {
    if (!this.storageData || !this.energy) {
      return
    }

    try {
      await this.storageData.setItem(ENERGY_STORAGE_KEY, this.energy.snapshot())
    } catch (err) {
      this.log.debugWarn('Could not store the energy totals: %s.', parseError(err))
    }
  }

  configureMatterAccessory(accessory) {
    // What this plugin published over Matter before the restart. Anything not published again is
    // taken away once the devices are set up, rather than left in the controller for ever.
    this.matter?.rememberCached(accessory)
  }
